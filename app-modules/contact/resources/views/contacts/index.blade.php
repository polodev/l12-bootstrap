<x-admin-dashboard-layout::layout>
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Contact Management</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Manage customer inquiries and messages</p>
                </div>
            ...
            <div id="filter_area" class="bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-3" style="display: block;">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                    <div>
                        <label for="department" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Filter by Department</label>
                        <select class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" name="department" id="department">
                    ...
                    <div>
                        <label for="has_reply" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Filter by Reply Status</label>
                        <select class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" name="has_reply" id="has_reply">
                    ...
                    <div>
                        <label for="page" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Filter by Page</label>
                        <input class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" type="text" name="page" id="page" placeholder="e.g., /contact, /about">
                    </div>
                    <div class="md:col-span-1">
                        <label for="search_text" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Quick Search</label>
                        <input class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" type="text" name="search_text" id="search_text" placeholder="Search by name, email, topic...">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="overflow-hidden">
            <table id="contacts-table" class="w-full divide-y divide-gray-200 dark:divide-gray-700">
    ...
    @push('scripts')
    <script>
        const current_route_name = 'contact::admin.contacts.index';
        ...
        $(document).ready(function() {
            ...
            var contactsTable = $('#contacts-table').DataTable({
                ...
                responsive: false,
                ajax: {
                    url: '{{ route('contact::admin.contacts.json') }}',
                    type: "POST",
                    data: function(d) {
                        d.department = $('#department').val();
                        d.has_reply = $('#has_reply').val();
                        d.page = $('#page').val();
                ...
                drawCallback: function() {
                    ...
                    $('#contacts-table').addClass('divide-y divide-gray-200 dark:divide-gray-700');
                    $('#contacts-table thead').addClass('bg-gray-50 dark:bg-gray-700');
                    $('#contacts-table thead th').addClass('px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider');
                    $('#contacts-table tbody').addClass('bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700');
                    $('#contacts-table tbody td').addClass('px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100');
                    
                    // Fix column alignment after draw
                    setTimeout(function() {
                        contactsTable.columns.adjust();
                    }, 100);
                }
            });
            
            // Filter change listeners
            var filterElements = ['#department', '#has_reply'];
            ...
            let searchTimeout;
            $('#search_text, #page').keyup(function() {
