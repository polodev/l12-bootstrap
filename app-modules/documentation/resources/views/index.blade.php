<x-admin-dashboard-layout::layout>
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Documentation Management</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Create and manage website documentation</p>
                </div>
                <div class="flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('documentation::admin.create') }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md hover:bg-blue-700">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div id="filter_area" class="bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-3" style="display: block;">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                    <div>
                        <label for="section" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Filter by Section</label>
                        <select class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" name="section" id="section">
                    ...
                    <div>
                        <label for="difficulty" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Filter by Difficulty</label>
                        <select class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" name="difficulty" id="difficulty">
                    ...
                    <div>
                        <label for="is_published" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Publication Status</label>
                        <select class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" name="is_published" id="is_published">
                    ...
                    <div>
                        ...
                        <input class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" type="text" name="search_text" id="search_text" placeholder="Search by title, content...">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="overflow-hidden">
            <table id="documentation-table" class="w-full divide-y divide-gray-200 dark:divide-gray-700">
    ...
    @push('scripts')
    <script>
        const current_route_name = 'documentation::admin.index';
        
        $(document).ready(function() {
            // DataTable configuration
            var documentationTable = $('#documentation-table').DataTable({
                processing: true,
                serverSide: true,
                searchDelay: 500,
                pageLength: 25,
                ...
                responsive: false,
                ajax: {
                    url: '{{ route('documentation::admin.json') }}',
                    type: "POST",
                    data: function(d) {
                        d.section = $('#section').val();
                        d.difficulty = $('#difficulty').val();
                ...
                language: {
                    ...
                    infoEmpty: "No documentation found",
                ...
                drawCallback: function() {
                    ...
                    $('#documentation-table').addClass('divide-y divide-gray-200 dark:divide-gray-700');
                    $('#documentation-table thead').addClass('bg-gray-50 dark:bg-gray-700');
                    $('#documentation-table thead th').addClass('px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider');
                    $('#documentation-table tbody').addClass('bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700');
                    $('#documentation-table tbody td').addClass('px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100');
                    
                    // Fix column alignment after draw
                    setTimeout(function() {
                        documentationTable.columns.adjust();
                    }, 100);
                }
            });
            
            // Filter change listeners
            var filterElements = ['#section', '#difficulty', '#is_published'];
            
            filterElements.forEach(function(element) {
                $(element).change(function(e) {
                    documentationTable.draw();
                    e.preventDefault();
                });
            });
            ...
            documentationTable.on('draw', function() {
                updateDataTableInfo();
            });
