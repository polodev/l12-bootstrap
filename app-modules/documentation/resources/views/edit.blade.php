    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex items-center justify-between">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Edit Documentation</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Update existing documentation</p>
                </div>
        ...
        <div class="p-6">
            <form action="{{ route('documentation::admin.update', $documentation->slug) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="section"
                            ...
                            @foreach ($sections as $key => $value)
                                <option value="{{ $key }}" {{ old('section', $documentation->section) == $key ? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="difficulty"
                            ...
                            @foreach ($difficulties as $key => $value)
                                <option value="{{ $key }}" {{ old('difficulty', $documentation->difficulty) == $key ? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" 
                               id="position"
                               name="position"
                               value="{{ old('position', $documentation->position) }}"
                               min="0"
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                ...
                <div>
                    ...
                    <textarea id="content"
                              ...
                              required>{{ old('content', $documentation->content) }}</textarea>
                    @error('content')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const easyMDE = new EasyMDE({
                ...
                autosave: {
                    enabled: true,
                    uniqueId: "documentation_edit_{{ $documentation->id }}",
                    delay: 1000,
                },
