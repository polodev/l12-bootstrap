...
class SslCommerzNotification extends AbstractSslCommerz
{
  ...
  private $error;
    ...
    {
      $this->config = config('sslcommerz.main-store');
      if ($store == 'zaytoon') {
        $this->config = config('sslcommerz.zaytoon-store');
      }
    ...
    public function orderValidate($post_data, $trx_id = '', $amount = 0, $currency = "BDT")
    {
      if (! is_array($post_data)) {
        $this->error = "Didn't get valid data from Payment Gateway";
        return false;
      }
  ...
  public function getPaymentDataByValidationID($val_id = null)
  {
    if (!$val_id) {
      return null;
    }
