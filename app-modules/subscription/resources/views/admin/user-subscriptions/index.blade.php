<x-admin-dashboard-layout::layout>
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        ...
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-4">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100">User Subscriptions</h2>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Manage user subscription records and status</p>
                </div>
            ...
            <div id="filter_area" class="bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg p-3" style="display: none;">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div>
                        <label for="plan_id" class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Filter by Plan</label>
                        <select class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" name="plan_id" id="plan_id">
                            <option value="">All Plans</option>
                            @foreach(\Modules\Subscription\Models\SubscriptionPlan::active()->ordered()->get() as $plan)
                    ...
                    <div>
                        ...
                        <input class="w-full px-2 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" type="text" name="search_text" id="search_text" placeholder="Search by user name or email...">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="overflow-hidden">
            <table id="subscriptions-table" class="w-full divide-y divide-gray-200 dark:divide-gray-700">
    ...
    <script>
        $(document).ready(function() {
            ...
            var subscriptionsTable = $('#subscriptions-table').DataTable({
                processing: true,
                serverSide: true,
                searchDelay: 500,
                pageLength: 25,
                ...
                responsive: false,
                ajax: {
                    url: '{{ route('subscription::admin.subscriptions.json') }}',
                    type: "POST",
                    headers: {
                ...
                drawCallback: function() {
                    ...
                    $('#subscriptions-table').addClass('divide-y divide-gray-200 dark:divide-gray-700');
                    $('#subscriptions-table thead').addClass('bg-gray-50 dark:bg-gray-700');
                    $('#subscriptions-table thead th').addClass('px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider');
                    $('#subscriptions-table tbody').addClass('bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700');
                    $('#subscriptions-table tbody td').addClass('px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100');
                    
                    // Fix column alignment after draw
                    setTimeout(function() {
                        subscriptionsTable.columns.adjust();
                    }, 100);
                }
            });
            
            // Filter change listeners
            var filterElements = ['#status', '#plan_id'];
            ...
            subscriptionsTable.on('draw', function() {
                updateDataTableInfo();
            });
