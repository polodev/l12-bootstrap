<x-customer-account-layout::layout>
    ...
    <div class="mb-6 flex items-center text-sm">
        ...
        <span class="text-gray-500 dark:text-gray-400">{{ __('messages.add_new_address') }}</span>
    </div>

    <!-- Page Title -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('messages.add_new_address') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('messages.add_new_address_description') }}</p>
    </div>
    ...
    <div class="space-y-6">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <div class="p-6">
                        <form action="{{ route('accounts.addresses.store') }}" method="POST" class="space-y-6">
                            @csrf
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="type" id="type" required
                                        ...
                                        <option value="home" {{ old('type') === 'home' ? 'selected' : '' }}>{{ __('messages.home') }}</option>
                                        <option value="work" {{ old('type') === 'work' ? 'selected' : '' }}>{{ __('messages.work') }}</option>
                                ...
                                <div>
                                    <x-forms.input :label="__('messages.address_name')" name="name" type="text"
                                        value="{{ old('name') }}" required placeholder="{{ __('messages.address_name_placeholder') }}" />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="address_line_1" id="address_line_1" rows="2" required
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300"
                                    placeholder="{{ __('messages.address_line_1_placeholder') }}">{{ old('address_line_1') }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="address_line_2" id="address_line_2" rows="2"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300"
                                    placeholder="{{ __('messages.address_line_2_placeholder') }}">{{ old('address_line_2') }}</textarea>
                            ...
                            <div class="flex justify-between pt-6 border-t border-gray-200 dark:border-gray-700">
                                ...
                                <x-button type="primary">{{ __('messages.save_address') }}</x-button>
                            </div>
                        </form>
                    </div>
                </div>
